...
public ModelAndView reply_community(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		mypageDAO dao = new mypageDAO();
		MemberDAO dao1 = new MemberDAO();
		 MyUtil util = new MyUtilBootstrap();
		HttpSession session = req.getSession();
		...
		List<BoardDTO> list = dao.replyList(offset, size, dto);
		
		 String cp = req.getContextPath();
	        String listUrl = cp + "/mypage/replylist";
	        String articleUrl = cp + "/mypage/replylist?page=" + currentPage;
	        
	        String paging = util.paging(currentPage, total_page, listUrl);
		
		ModelAndView mav = new ModelAndView("mypage/reply_community");
		mav.addObject("articleUrl", articleUrl);
		mav.addObject("paging", paging);
