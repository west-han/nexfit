					...
					<div class="col-sm-7">
						...
						<div class="body-title">
							<h2 style="font-family: 'nexon lv2 medium';"
								class="text-with-border">등록된 챌린지</h2>
						</div>
						<div class="body-main" style="font-family: nexon lv1;">
							<form name="listForm" method="post">
								...
								<c:if test="${sessionScope.member.userId == 'admin'}">
									<button type="button" class="btn btn-light" id="btnDeleteList"
										title="삭제" style="float: left;">
										<i class="bi bi-trash"></i>
									</button>

									<button type="button" class="btn btn-light"
										style="float: right;"
										onclick="location.href='${pageContext.request.contextPath}/challenge/write';">글올리기</button>
								</c:if>
								<table class="table table-hover board-list">
									<thead class="table-light">
										<tr>
											<c:if test="${sessionScope.member.userId=='admin'}">
												<th class="chk" width="100px"><input type="checkbox"
													class="form-check-input" name="chkAll" id="chkAll">
												</th>
											</c:if>
											...
											<tr>
												<c:if test="${sessionScope.member.userId=='admin'}">
													<td><input type="checkbox" class="form-check-input"
														name="nums" value="${dto.challengeId}"></td>
												</c:if>
												<td>${dataCount - (page-1) * size - status.index}</td>
												<td class="left"><a
													href="${articleUrl}&challengeId=${dto.challengeId}"
													class="text-reset">${dto.ch_subject}</a></td>
												<td>${dto.fee}</td>
											</tr>
										</c:forEach>
									</tbody>
							...
							<div class="page-navigation"
								...
								<div class="col text-start">
									<button type="button" class="btn btn-light"
										onclick="location.href='${pageContext.request.contextPath}/challenge/list';">
